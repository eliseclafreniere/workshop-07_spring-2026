...
H~0~: Elevation (m) does not predict temperature (°C) in Sonadora.

H~A~: Elevation (m) predicts temperature (°C) in Sonadora.
...
# creating summary of temperature_model data
summary(temperature_model)
```

For each meter of elevation gain, you would expect a decrease in temperature equivalent to 0.006 $\pm$ 0.0003 (SE) °C.
...
ggpredict(
  temperature_model,           # model object
  terms = "elevation_m[900]"   # predictor (in quotation marks)
                               # predictor value in brackets
)
```
